...
class MixedRecordingManager {
  ...
  _buildFilterComplex(videoCount, audioCount, targetW, targetH) {
    ...
    for (let i = 0; i < videoCount; i += 1) {
      scaleLabels.push(`[${i}:v]scale=${cellW}:${cellH}:force_original_aspect_ratio=decrease:eval=frame,pad=${cellW}:${cellH}:(ow-iw)/2:(oh-ih)/2:color=black[v${i}]`)
  ...
  async startMixedRecording({ room_id, user_name, width = 1920, height = 1080 }) {
    ...
    const ffArgs = [
      ...
      '-fflags', '+genpts', '-analyzeduration', '15000000', '-probesize', '15000000'
    ]
    ...
    ffArgs.push(
      '-c:v', 'libvpx', '-b:v', '2500k', '-crf', '30', '-r', '30',
      '-pix_fmt', 'yuv420p',
      '-deadline', 'realtime',
  ...
  async stopMixedRecording({ recording_id }) {
    ...
    try { st.ff.kill('SIGINT') } catch {}
    for (const c of st.consumers) { try { await c.close() } catch {} }
    for (const t of st.transports) { try { await t.close() } catch {} }
    try {
      // Cleanup SDP files
